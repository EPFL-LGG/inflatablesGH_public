...
struct Nondimensionalization {
    ...
    Nondimensionalization(const InflatableSheet &sheet) {
        ...
        // If the sheet is using single line fusing curve (no fused region), then use the total length of the fusing lines as the fusedArea, total area of the sheet over the total length as wallWidth.
        if (sheet.using_single_fusing_line()) {
            fusedArea = 0.0;
            wallWidth = 0.0;
            using IdxPolyline = std::vector<size_t>;
            using FusingPolylines =  std::pair<std::vector<IdxPolyline>, std::vector<IdxPolyline>>;
            FusingPolylines fpl = sheet.getFusingPolylines();
            for (auto &pl : fpl.first) {
                // Ignore the first outer boundary since it stays constant with respect to the spatial frequency.
                for (size_t i = 1; i < pl.size() - 1; ++i) {
                    auto &p0 = m.node(pl[i])->p;
                    auto &p1 = m.node(pl[i + 1])->p;
                    fusedArea += (p1 - p0).norm();
                }
            }
            for (auto &pl : fpl.second) {
            ...
            wallWidth = 0.1 * sheet.mesh().volume() / fusedArea;
            return;
        }
        fusedArea = 0.0;
