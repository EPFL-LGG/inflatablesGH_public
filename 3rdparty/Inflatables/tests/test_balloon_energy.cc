...
void runTest(const M2d &C, double fd_eps) {
    ...
    auto dEnergyAt = [&](const M2d &X, const M2d &perturb) {
        return IBE(X).denergy_dC(perturb);
    };
    ...
    double an_delta_energy = balloon.denergy_dC(perturb);
    ...
    for (size_t i = 0; i < 2; ++i) {
        for (size_t j = 0; j < 2; ++j) {
            ...
            an_delta_grad_energy(i, j) = balloon.d2energy_dC(ei_otimes_ej, perturb);
...
int main(int argc, const char *argv[]) {
    ...
    auto evalHessAtJ = [&](const M32d &Jeval, const M32d &perturb_a, const M32d &perturb_b) {
        ...
        IBE ibe(C);
        return ibe.d2energy_dC(dC_a, dC_b) + ibe.denergy_dC(d2C_ab);
    };
