...
// Evaluate a triangle's stripe scalar field at particular barycentric coordinates
double evalCross(const std::array<DDG::Complex, 3> &g, double nu, double nv, const Eigen::Vector3d &b /* barycentric coordinates */, double w) {
    double theta = g[0].re * b[0] +
                   g[1].re * b[1] +
                   g[2].re * b[2];
    ...
    //      equivalent to pi - acos(cos(theta))     (acos returns value in [0, pi]
    {
        ...
        // if (origTheta < M_PI) 
        return abs(theta - (sin(gamma) + 1.5));
        // else return  abs(theta + sin(gamma));;
         ...
         if (k == 0) {
             ...
             subdivide_triangle(nsubdiv, p[0], p[1], p[2], indexForPoint,
                                [&](const Eigen::Vector3d &p_sub, double b0, double b1, double b2) {
                                    ...
                                    //  stripeField.push_back(evalCross(g, nu, nv, Eigen::Vector3d(b0, b1, b2), (b0 * w[0] + b1 * w[1] + b2 * w[2]))); // linearly interpolated half-wall-width
         ...
         else // singular triangle
         {
             ...
             for (int offset = 0; offset < 3; ++offset) {
                 ...
                 auto newPt = [&](const Eigen::Vector3d &p, double b0, double b1, double b2) {
                    ...
                    //  stripeField.push_back(evalCross(g, nu, nv, Eigen::Vector3d(b0, b1, b2), (b0 * w_ijk[offset] + b1 * w_ijk[next] + b2 * wm))); // linearly interpolated half-wall-width
                     return outVertices.size() - 1;
                 };
