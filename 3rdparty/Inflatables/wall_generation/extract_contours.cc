                      ...
                      std::vector<std::pair<size_t, size_t>> &outEdges)
{
    ...
    for (auto be : mesh.boundaryEdges()) {
        ...
        if (getBoundaryContourPt(start) == NONE) {
            std::cout<<"Couldn't find contour-boundary intersection"<<std::endl;
            // Add boundary loop only.
            polylines.emplace_back();
            auto curr = start;
            ...
            assert(curr == start);
            // We have now returned to the first boundary edge 
    ...
    for (std::vector<Eigen::Vector3d> &line : polylines) {
        ...
        if (targetEdgeSpacing < std::numeric_limits<double>::max()) {
            ...
            for (size_t i = 1; i < n; ++i) { // Traverse original curve
                ...
                if (distToNextPoint <= 0) throw std::runtime_error("Extract contour: Negative distance to next point");
            }
            traversalFinished:
