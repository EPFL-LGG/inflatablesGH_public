...
def allGradientNorms(obj, freeVariables = None):
    if freeVariables is None:
        freeVariables = np.arange(obj.numVars(), dtype=np.int64)
...
def prototypeScaleNormalization(P, placeAtopFloor = False, objectScale = 300, reorient = False):
...
def normalOffset(polygon, dist):
    ...
    if (isinstance(offsetResult, shp.Polygon)):
        ...
        return list(offsetResult.geoms)
...
def getBoundary(polygon, getAll = False):
    ...
    if result.geom_type == 'LineString':
        if getAll: return [np.array(result.coords)]
        return np.array(result.coords)
    if result.geom_type == 'MultiLineString':
        allBoundaries = sorted([np.array(line.coords) for line in result.geoms], key=lambda a: -len(a))
...
def reconnectPolygons2(inputPolygons, originatingPolygon, fuseWidth, includeExtensions=False):
    ...
    originatingPolygon = shp.Polygon(originatingPolygon)
    bridges = [p for p in originatingPolygon.difference(shapely.ops.unary_union(polygons)).geoms if p.boundary.length > 3 * fuseWidth]
    outputPolylines = inputPolygons.copy()
    for b in bridges:
        ...
        if (distances[closest[1]] < fuseWidth / 2):
            ...
            outputPolylines.append(np.array([np.array(p0.coords)[0], np.array(p1.coords)[0]]))
        elif includeExtensions:
            if (b.boundary.length > 4 * fuseWidth):
                bdryPts = np.array(b.boundary.coords)
                _, p0 = shapely.ops.nearest_points(b, polygons[closest[0]])
                b_to_p0 = scipy.spatial.distance.cdist(np.array(p0.coords)[:, :2], bdryPts[:, 0:2])[0]
                ...
                if (b_to_p0[farthest] > 4 * fuseWidth):
                    ...
                    outputPolylines.append(np.array([np.array(p0.coords)[0, :2], bdryPts[farthest, 0:2]]))
    return outputPolylines
