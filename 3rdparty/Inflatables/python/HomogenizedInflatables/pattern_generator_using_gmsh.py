...
def get_cosine_dash(h, avg_len_boundary, avg_len_embeddings, amplitude, dash_point, return_line_segments = False):
    ...
    ipu = inflation.InflatablePeriodicUnit(m, fusing_data, epsilon = 1e-9)
...
def get_square_pillow(a, b, avg_len_boundary = 0.1, avg_len_embeddings = 0.1, scale = 1):
    if a != b:                
        ipu = inflation.InflatablePeriodicUnit(*get_square_pillow_helper(a, b, avg_len_boundary, avg_len_embeddings), epsilon = 1e-5)
    else:
        scale = scale / 5
