...
def get_sampled_bending_stiffness(ipu, resolution, optimizer, minAlpha, maxAlpha, hessianShift = 1e-10, fixedVars = np.array([])):
    sampled_alpha = np.linspace(minAlpha, maxAlpha, resolution)
    bending_stiffness_sample_alpha = inflation.getBendingStiffness(ipu, sampled_alpha, optimizer, hessianShift, fixedVars)
    return bending_stiffness_sample_alpha, sampled_alpha
...
def get_sampled_bending_stiffness_using_bases(ipu, resolution, optimizer, minAlpha, maxAlpha, hessianShift = 1e-10, fixedVars = np.array([])):
    sampled_alpha = np.linspace(minAlpha, maxAlpha, resolution)
    bending_stiffness_sample_alpha, stiffness_coefficient = inflation.getBendingStiffnessUsingBases(ipu, sampled_alpha, optimizer, hessianShift, fixedVars)
...
class stretching_stiffness_class:
    ...
    def setVars(self, x):
        ...
        optimizer = inflation.get_inflation_optimizer(self.ipu, self.ipu.getStretchingStiffnessFixedVars(), opts, callback=cb, hessianShift = 0)
    ...
    def secondDerivative(self): return inflation.debugTangentElasticityTensor(self.ipu, [], self.optimizer, 0, self.ipu.getStretchingStiffnessFixedVars())[self.index][self.index]
...
class bending_stiffness_class:
    ...
    def setVars(self, kappa):
        ...
        def cb(it):
            if it % framerate == 0:
                self.viewer.update(scalarField=utils.getStrains(self.sheet)[:, 0])
        optimizer = inflation.get_inflation_optimizer(self.ipu, fixedVars, opts, callback=cb, hessianShift = 0)
        cr = optimizer.optimize()
        print(cr.success)
    ...
    def secondDerivative(self): return inflation.getBendingStiffness(self.ipu, np.array([self.ipu.getVars()[-1]]), self.optimizer, 0, [self.ipu.get_average_z_idx(), periodic_unit_helper.get_center_fixedVars(self.ipu.ipu)[0], periodic_unit_helper.get_center_fixedVars(self.ipu.ipu)[1], self.ipu.numVars() - 2, self.ipu.numVars() - 1])
...
class bending_total_gradient:
    ...
    def setVars(self, kappa):
        ...
        def cb(it):
            if it % framerate == 0:
                self.viewer.update(scalarField=utils.getStrains(self.sheet)[:, 0])
        optimizer = inflation.get_inflation_optimizer(self.ipu, fixedVars, opts, callback=cb, hessianShift = hessianShift)
        cr = optimizer.optimize()
        print(cr.success)
...
def get_az_ipu_from_ipu(ipu, m, fusedVtx, useTFT, disableFusedRegionTFT = False):
    ...
    az_ipu = inflation.InflatableMidSurfacePeriodicUnit(m, fusedVtx, epsilon = 1e-9)
