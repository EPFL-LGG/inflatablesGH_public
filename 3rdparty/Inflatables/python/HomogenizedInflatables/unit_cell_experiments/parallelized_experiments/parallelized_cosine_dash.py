...
def run_mirror_cosine_dash(amp, r, angle):
    ...
    if (os.path.isfile('{}/experiment_result.json'.format(result_folder))):
        ...
        if data["Planar equilibrium"] == 1:
            ...
            return
    print("running {:.2f}_{:.2f}_{:.2f}".format(amp, r, angle))
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, "{:.2f}_{:.2f}_{:.2f}".format(amp, r, angle), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    amplitudes = np.linspace(0, 0, 1)
    # amplitudes = [amplitudes[0]]
    radius = np.linspace(0.5, 2.3, 19)
        ...
        for r in radius:
            for angle in angles:
                ...
                with open('{}/experiment_result.json'.format(result_folder), 'r') as fp:
                    ...
                    curr_json['name'] = get_label(amp, r, angle)
