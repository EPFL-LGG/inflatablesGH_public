...
name = "square_with_ellipse_hole_angle_width_with_height_{}".format(ih)
time_stamp = time.strftime("%Y_%m_%d_%H_%M")
time_stamp = "2024_01_17_09_06"
...
def get_label(angle, width):
    return "{:.2f}_{:.2f}".format(angle, width)

def run_square_with_ellipse_holes(angle, width):
   
    result_folder = "{}/{}".format(base_folder, get_label(angle, width))
    ...
    if (os.path.isfile('{}/experiment_result.json'.format(result_folder))):
        ...
        if data["Planar equilibrium"] == 1:
            return
    print("running {:.2f}_{:.2f}".format(angle, width))

    h = 5
    iw = width
    m, marker = pattern_generator_using_gmsh.get_pattern_with_elliptic_holes(h, iw, ih, avg_len, angle = angle)
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, get_label(angle, width), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    ...
    for a in angles:
        for w in widths:
            result_folder = "{}/{}".format(base_folder, get_label(a, w))
