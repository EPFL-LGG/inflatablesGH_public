...
name = "square_with_holes"
time_stamp = time.strftime("%Y_%m_%d_%H_%M")
time_stamp = "2024_01_12_23_26"
...
def run_square_with_holes(radius, angle):
    ...
    h = 5
    m, marker = pattern_generator_using_gmsh.get_pattern_with_rectangle_holes(h, ih, iw, avg_len)
...
if __name__ == '__main__':
    angles = np.linspace(45, 75, 16)[:13]
    radius = np.linspace(0.5, 4, 8)
    ...
    with multiprocessing.Pool(logical_core_count) as p:
        p.starmap(run_square_with_holes, args)
