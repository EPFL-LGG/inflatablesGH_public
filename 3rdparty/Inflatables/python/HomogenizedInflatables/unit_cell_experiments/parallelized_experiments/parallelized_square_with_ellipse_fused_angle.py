...
name = "square_with_ellipse_fused_angle_height_{}_width_{}".format(ih, iw)
time_stamp = time.strftime("%Y_%m_%d_%H_%M")
time_stamp = "2024_01_14_20_38"
...
def run_square_with_ellipse_holes(angle):
   
    result_folder = "{}/{}".format(base_folder, get_label(angle))
    ...
    if (os.path.isfile('{}/experiment_result.json'.format(result_folder))):
        ...
        if data["Planar equilibrium"] == 1:
            return
    print("running {:.2f}".format(angle))

    h = 5
    m, marker = pattern_generator_using_gmsh.get_pattern_with_elliptic_fused(h, iw, ih, avg_len, angle = angle)
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, get_label(angle), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(logical_core_count) as p:
        p.map(run_square_with_ellipse_holes, angles)
    ...
    experiment_logs = []
    for a in angles:
        result_folder = "{}/{}".format(base_folder, get_label(a))
