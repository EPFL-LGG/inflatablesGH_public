...
name = "square_with_disk_holes"
time_stamp = time.strftime("%Y_%m_%d_%H_%M")
time_stamp = "2024_01_13_16_48"
...
def run_square_with_holes(radius):
   
    result_folder = "{}/{}".format(base_folder, get_label(radius))
    ...
    if (os.path.isfile('{}/experiment_result.json'.format(result_folder))):
        ...
        if data["Planar equilibrium"] == 1:
            return
    print("running {}".format(get_label(radius)))

    h = 5
    m, marker = pattern_generator_using_gmsh.get_pattern_with_circular_holes(h, radius, avg_len)
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, get_label(radius), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(logical_core_count) as p:
        p.map(run_square_with_holes, radius)
    ...
    experiment_logs = []
    for r in radius:
        result_folder = "{}/{}".format(base_folder, get_label(r))
