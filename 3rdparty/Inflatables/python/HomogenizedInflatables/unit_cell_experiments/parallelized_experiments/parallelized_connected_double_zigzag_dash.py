...
name = "connected_double_zigzag_dash"
time_stamp = time.strftime("%Y_%m_%d_%H_%M")
time_stamp = "2024_01_12_10_25"
...
def run_double_zigzag_dash(r, angle):
    ...
    h = 5
    dash_point = np.array([np.cos(angle / 180 * np.pi), np.sin(angle / 180 * np.pi)]) * r + np.array([0, 5 / 4])
    m, marker = pattern_generator_using_gmsh.get_connected_double_zigzag_dash(h, avg_len, avg_len, dash_point = dash_point)
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, get_label(r, angle), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    # h / 4 * np.sqrt(2)
    radius = np.linspace(0.5, 1.8, 14)
    angles = np.linspace(45, 90, 16)[:11]
    ...
    with multiprocessing.Pool(logical_core_count) as p:
        p.starmap(run_double_zigzag_dash, args)
