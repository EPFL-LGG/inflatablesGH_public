...
name = "square_with_ellipse_hole_height_{}".format(height)
...
def get_label(width):
    return "{:.2f}".format(width)

def run_square_with_ellipse_holes(width):
    iw = width
    ih = height
   
    result_folder = "{}/{}".format(base_folder, get_label(width))
    ...
    experiment_log = experiment_helper.run_experiment(ipu, m, fusedVtx, stiffness_pressure, scale_factor_pressure, name, get_label(width), result_folder, useTFT = useTFT, use_low_pressure=False)
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(logical_core_count) as p:
        p.map(run_square_with_ellipse_holes, widths)
    ...
    experiment_logs = []
    for w in widths:
        result_folder = "{}/{}".format(base_folder, get_label(w))
