...
def visualizeChannelOrientation(param, quiver=QuiverVisualization.PER_TRI, orientationHue=True, width=14, height=14):
    ...
    plt.quiver(quiverPts[:, 0], quiverPts[:, 1],
               ...
               pivot='mid', scale=50.0,
               headaxislength=0, headlength=0, headwidth=0)
    #plt.triplot(uvTriang, 'k-', lw=0.25)
    plt.tight_layout()
    # plt.savefig('test.png')

def visualizeChannelOrientationWithIsotropicPoints(param, quiver=QuiverVisualization.PER_TRI, orientationHue=True, width=14, height=14, use_x_axis = False):
    uvTriang = uvTriangulation(param)
    diff = abs(param.getAlphas() - param.getBetas())
    indices = np.where((diff < 1e-3) * (diff > -1e-3))[0]
    m = param.mesh().copy()
    vertices = np.zeros(m.vertices().shape)
    vertices[:,:2] = param.uv()
    m.setVertices(vertices)
    umb_x, umb_y = m.barycenters()[indices][:,0], m.barycenters()[indices][:,1]
    ...
    if (quiver == QuiverVisualization.PER_TRI):
        ...
        quiverPts = triCenters
        shading='flat'
        stretchAngleOffset = param.get_stretch_angle_offset_from_pattern_params(param.getMatInfoArgs())
    if (quiver == QuiverVisualization.PER_VTX):
        leftStretchAngle = param.perVertexLeftStretchAngles()
        quiverPts = uv
        shading='gouraud'
    ...
    if use_x_axis:
        if quiver == QuiverVisualization.PER_VTX:
            raise ValueError("Angle offset is not implemented per vertex")
        offsetted_channelOrientation = channelOrientation + stretchAngleOffset
        channelOrientation = np.array(list(channelOrientation) + list(offsetted_channelOrientation))
        quiverPts = np.array(list(quiverPts) + list(quiverPts))
    if orientationHue: plt.tripcolor(uvTriang, hueFromHalfAngle(channelOrientation), cmap=matplotlib.cm.hsv, vmin=0, vmax=1, shading=shading)
    plt.quiver(quiverPts[:, 0], quiverPts[:, 1],
               np.cos(channelOrientation), np.sin(channelOrientation),
               pivot='mid', scale=100.0,
               headaxislength=0, headlength=0, headwidth=0)
    print(min(stretchAngleOffset), max(stretchAngleOffset))
    plt.scatter(umb_x, umb_y, c= 'r', s = 15)
    #plt.triplot(uvTriang, 'k-', lw=0.25)
    plt.tight_layout()
    # plt.savefig('test.png')
...
def visualize(param, width=16, height=7):
    ...
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    plt.tripcolor(uvt, hueFromHalfAngle(param.leftStretchAngles()), cmap=matplotlib.cm.twilight, vmin=0, vmax=1)
    plt.colorbar()
...
def visualize_both(param, width=16, height=7, show_main = False, path = None):
    plt.figure(figsize=(width, height))
    uvt = uvTriangulation(param)
    ...
    total_figures = 3 if show_main else 5
    
    
    plt.subplot(1,total_figures, 1)
    plt.title('Stretch Orientation')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    plt.tripcolor(uvt, hueFromHalfAngle(param.leftStretchAngles()), cmap=matplotlib.cm.twilight, vmin=0, vmax=1)
    ...
    #     plt.colorbar()
    #     plt.scatter(umb_x, umb_y, c= 'k', s = 10)
    # else:
    #     plt.subplot(1, 6, 2)
    #     plt.title('Barriers')
    #     plt.axis('equal')
    #     plt.triplot(uvt, 'k-', lw=0.25)
    #     plt.tripcolor(uvt, param.getBarriers(), vmin=0, vmax=1, cmap=matplotlib.cm.viridis)
    #     plt.colorbar()
    #     # plt.scatter(umb_x, umb_y, c= 'k', s = 10)

    plt.subplot(1, total_figures, 2)
    plt.title('Stretch Factor 1')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    # plt.scatter(umb_x, umb_y, c= 'k', s = 10)
    plt.tripcolor(uvt, param.getAlphas(), vmin=param.alphaMin, vmax = param.alphaMax, cmap=matplotlib.cm.viridis)
    plt.tight_layout()
    plt.colorbar()
    # plt.scatter(umb_x, umb_y, c= 'k', s = 10)

    plt.subplot(1,total_figures, 3)
    plt.title('Stretch Factor 2')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    # plt.scatter(umb_x, umb_y, c= 'k', s = 10)
    plt.tripcolor(uvt, param.getBetas(), vmin=param.betaMin, vmax = param.betaMax, cmap=matplotlib.cm.viridis)
    plt.tight_layout()
    plt.colorbar()

    if not show_main:
        try:
            plt.subplot(1,total_figures, 4)
            plt.title('Kappa 1')
            plt.axis('equal')
            plt.triplot(uvt, 'k-', lw=0.25)
            plt.tripcolor(uvt, param.getKappa(0), cmap=matplotlib.cm.viridis)
            plt.tight_layout()
            plt.colorbar()
        except:
            pass
        ...
        try:
            plt.subplot(1,total_figures, 5)
            plt.title('Kappa 2')
            plt.axis('equal')
            plt.triplot(uvt, 'k-', lw=0.25)
            plt.tripcolor(uvt, param.getKappa(1), cmap=matplotlib.cm.viridis)
            plt.tight_layout()
            plt.colorbar()
        except:
            pass
...
def visualize_pattern(param, width=16, height=7, showBarriers = False, num_pattern_vars = 2, path = None):
    plt.figure(figsize=(width, height))
    uvt = uvTriangulation(param)
    ...
    plt.subplot(1, 4+num_pattern_vars, 1)
    plt.title('Stretch Orientation')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    plt.tripcolor(uvt, hueFromHalfAngle(param.leftStretchAngles()), cmap=matplotlib.cm.twilight, vmin=0, vmax=1)
    ...
    if not showBarriers:
        plt.subplot(1, 4+num_pattern_vars, 2)
        plt.title('Isotropic points')
        plt.axis('equal')
        plt.triplot(uvt, 'k-', lw=0.25)
        plt.tripcolor(uvt, hueFromHalfAngle(param.leftStretchAngles()), cmap=matplotlib.cm.twilight, vmin=0, vmax=1)
        plt.colorbar()
    ...
    plt.subplot(1, 4+num_pattern_vars, 3)
    plt.title('Stretch Factor 1')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    # plt.scatter(umb_x, umb_y, c= 'k', s = 10)
    ...
    plt.subplot(1,4+num_pattern_vars, 4)
    plt.title('Stretch Factor 2')
    plt.axis('equal')
    plt.triplot(uvt, 'k-', lw=0.25)
    # plt.scatter(umb_x, umb_y, c= 'k', s = 10)
    ...
    nt = m.numElements()
    for pi in range(num_pattern_vars):
        plt.subplot(1,4+num_pattern_vars, 4+pi + 1)
        plt.title('Pattern Param' + str(pi +1))
        plt.axis('equal')
        plt.triplot(uvt, 'k-', lw=0.25)
        # plt.scatter(umb_x, umb_y, c= 'k', s = 10)
        
        plt.tripcolor(uvt, param.getVars()[param.stretchOffset() + pi*nt:param.stretchOffset() + (pi+1)*nt], cmap=matplotlib.cm.viridis, vmin = param.patternParamBounds[pi][0], vmax = param.patternParamBounds[pi][1])
        plt.tight_layout()
        plt.colorbar()
...
def plot_2d_mesh(m, width=7, height=7, pointList=[], segmentList=[], additionalScatterPts = np.zeros((0, 1)), triEdgeWidth=0.25, bbox=None, markerSize=5**2, path = None):
...
def plot_polylines(polylines, width=7, height=7, bbox=None, physicalLineWidth=None, highlightMultipolygons=False, colorOverride=None, ax = None, path = None):
    """
...
def plot_line_segments(pts, edges, width=7, height=7, bbox=None, path = None, color = None):
    plot_polylines(break_into_polylines(pts, edges),
                   width=width, height=height, bbox=bbox, path = path, colorOverride=color)
...
def singularValueHistogramBoth(param, width=7, height=7, bins=500):
    plt.figure(figsize=(width, height))
    plt.rc('text', usetex=False)
    plt.hist(np.concatenate((param.getAlphas(),
                             param.getBetas())), bins=bins)
    plt.title('Distribution of Singular Values σ0, σ1')

def scalarFieldPlotFast(vertices, tris, field, nlevels = 16, width=14, height=14, customMin = None, customMax = None, showEdges = False, cmap = matplotlib.cm.viridis, path = None):
...
def scalarFieldPlotZeroContourFast(vertices, tris, field, width=14, height=14, customMin = None, customMax = None, showEdges = False, cmap = matplotlib.cm.viridis):
    tri = mtri.Triangulation(vertices[:, 0], vertices[:, 1], tris)
    plt.figure(figsize=(width, height))
    plt.axis('equal')
    if (customMin is None): customMin = np.min(field)
    if (customMax is None): customMax = np.max(field)
    if showEdges: plt.triplot(tri, 'k-', lw=0.25)
    plt.tricontour(tri, field, [1e-1], cmap=cmap)
    plt.tight_layout()
...
from enum import Enum
class ViewerWithSurface(tri_mesh_viewer.TriMeshViewer):
    class ViewType(Enum):
        STRUCTURE = 1
        SURFACE = 2
    ...
    def __init__(self, structure, target_surf, *args, **kwargs):
        ...
        self.viewOptions = {self.ViewType.STRUCTURE: self.ViewOption(True, 'green'),
    ...
    def subview(self, viewType):
        if (viewType == self.ViewType.STRUCTURE): return self.structureView()
...
def set_surface_view_options(view, color = None, surface_color = 'gray', sheet_transparent = None, surface_transparent = True):
    view.viewOptions = {view.ViewType.STRUCTURE: view.ViewOption(sheet_transparent, color),
...
def optimizationViewers(isheet, width, height, offscreen):
    if isheet.using_single_fusing_line():
        flat_viewer = None
    else:
        try:
            ...
            deploy_colors = None
    Viewer = tri_mesh_viewer.OffscreenTriMeshViewer if offscreen else tri_mesh_viewer.TriMeshViewer
    deploy_viewer = Viewer(isheet, width=width, height=height)
...
def renderOptimizationIterateData(tas_in_path_fmt, out_path_prefix, opt_report, width, height, deployCam = None, flatCam = None, upscale=2, maxFrames = np.inf):
    ...
    if   flatCam is not None and flat_viewer is not None:   flat_viewer.setCameraParams(  flatCam)
    if deployCam is not None: deploy_viewer.setCameraParams(deployCam)

    if flat_viewer is not None:
...
def renderOptimizationIterates(sheet_opt, out_path_prefix, **customKWArgs):
    ...
    width, height = sheet_opt.deploy_viewer.getSize()
...
class TriMeshViewerWithSurface(TriMeshViewer):
    class ViewType(Enum):
        TRIMESH = 1
        SURFACE = 2
    ...
    def __init__(self, tri_mesh, target_surf, *args, **kwargs):
        if (isinstance(target_surf, str)):
            target_surf = mesh.Mesh(target_surf)

        super().__init__(tri_mesh, *args, **kwargs)
        self.surfView = TriMeshViewer(target_surf, superView=self)

        self.averagedMaterialFrames = True
        self.averagedCrossSections  = True

        self.viewOptions = {self.ViewType.TRIMESH: self.ViewOption(False, 'green'),
                            self.ViewType.SURFACE: self.ViewOption(True, 'lightgray')}
        self.applyViewOptions()
    ...
    def subview(self, viewType):
        if (viewType == self.ViewType.TRIMESH): return self.trimeshView()
        if (viewType == self.ViewType.SURFACE): return self.surfaceView()
        else: raise Exception('unknown view type')
