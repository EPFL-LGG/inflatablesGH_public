...
def shapely_to_numpy_polylines(shapely_polygons):
    def deshapelyed(p):
        ...
        if isinstance(p, shp.Polygon): 
            return [polyline for polyline in ([np.array(p.exterior.coords)] + [np.array(h.coords) for h in p.interiors])] # pure list of polylines
...
def write_design_file(outPath, polylines_and_samples, scale, channelMargin, tabMargin,
    ...
    if smartOuterChannelForISheet is not None:
        ...
        outerAirChannelPolygons = [shapely.ops.unary_union([sheetRegion] + list(bypasses.geoms))]
    else:
    ...
    scaledOutputCurves = [[scale * np.asarray(P) for P in c] if isinstance(c, list) else scale * c for c in outputCurves]
...
def writeFabricationData(outPath, origDesign, optDesign, iwv, targetSurf, uv,
    ...
    for name, designMesh in zip([ 'orig', 'opt'], [ origDesign, optDesign]):
