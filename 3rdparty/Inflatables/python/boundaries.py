...
def getOuterBoundaryVars(isheet):
    ...
    outerLoopBdry = max(m.boundaryLoops(), key=arclen)
    return [isheet.varIdx(0, BV[bvi], c) for bvi in outerLoopBdry for c in range(3)]

def getWallVtxOnOuterBoundary(isheet):
    m = isheet.mesh()
    V = m.vertices()
    BV = m.boundaryVertices()
    arclen = lambda l: np.linalg.norm(np.diff(V[BV[np.array(l)]], axis=0), axis=1).sum()
    outerLoopBdry = max(m.boundaryLoops(), key=arclen)
    outerLoopBdryVertices = BV[outerLoopBdry]
    wallVtxOnBoundary = np.zeros(len(isheet.wallVertices()), dtype=bool)
    for i, vx in enumerate(isheet.wallVertices()):
        if vx in outerLoopBdryVertices:
            wallVtxOnBoundary[i] = True
    return wallVtxOnBoundary
