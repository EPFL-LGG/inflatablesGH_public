...
struct KnitroProblem : public knitro::KNProblem {
        ...
        : Base(numParams, numConstraints)
    {
        ...
        Base::setHessEvalCallback(&callbackEvalHess);
    ...
    static int callbackEvalHess(KN_context_ptr             /* kc */,
                                CB_context_ptr             /* cb */,
                                KN_eval_request_ptr const  evalRequest,
                                KN_eval_result_ptr  const  evalResult,
                                void              * const  userParams) {
        if (evalRequest->type != KN_RC_EVALH) throw std::runtime_error("Unimplemented Hessian evaluation type");
        return ((Derived *)(userParams))->evalHess(evalRequest->x, *(evalRequest->sigma), evalRequest->lambda,
                                                      evalResult->hess);
    }
