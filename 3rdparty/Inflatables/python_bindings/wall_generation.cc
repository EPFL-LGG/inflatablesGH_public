...
PYBIND11_MODULE(wall_generation, m) {
    ...
    m.def("evaluate_cross_field_custom_pattern", 
        [](const Eigen::MatrixX3d &vertices,
            const Eigen::MatrixX3i &triangles,
            const std::vector<double> &stretchAngles,
            ...
            const size_t nsubdiv, bool glue) {
                std::tuple<Eigen::MatrixX3d,
                           Eigen::MatrixX3i,
                           Eigen::VectorXd, 
                           std::vector<std::array<Eigen::Vector3d, 2>>> result;
                std::vector<double> stripeField;
                evaluate_cross_field_custom_pattern(vertices, triangles, stretchAngles, patternParams, patternFunc, patternPolylineFunc, frequency, margin,
                                        std::get<0>(result), std::get<1>(result), stripeField, std::get<3>(result),
                                        nsubdiv, glue);
                std::get<2>(result) = Eigen::Map<Eigen::VectorXd>(stripeField.data(), stripeField.size());
                return result;
            },
            py::arg("vertices"), py::arg("triangles"),
            py::arg("stretchAngles"), py::arg("patternParams"), py::arg("patternFunc"), py::arg("patternPolylineFunc"), py::arg("frequency") = 130.0 /* default frequency from Keenan's code */, py::arg("margin") = 0.1,
            py::arg("nsubdiv") = 3, py::arg("glue") = true);
