...
### Installing `ipopt`/`cyipopt` (optional)
First make sure your pkgconfig can find `coinhsl.pc` and `ipopt.pc` once they
are installed; assuming you're installing everything with prefix `$HOME/usr`,
this means adding the following to your environment:
```
export PKG_CONFIG_PATH=$HOME/usr/lib/pkgconfig:$PKG_CONFIG_PATH
```

Build and install `HSL` for `ipopt`. The following assumes we have built
and installed metis as part of SuiteSparse in `$SUITESPARSE_ROOT`:
```
git clone https://github.com/coin-or-tools/ThirdParty-HSL
# download HSL from http://www.hsl.rl.ac.uk/ipopt/ and rename to coinhsl inside ThirdParty-HSL
# be sure to download the full stable release version with an academic license (not the limited `archive` version which omits ma57, etc.)
./configure --prefix=$HOME/usr --with-metis-lflags="-L$SUITESPARSE_ROOT -lmetis" -with-metis-cflags="-I$SUITESPARSE_ROOT/include"
make install
```

Install `ipopt`:
```
wget https://github.com/coin-or/Ipopt/archive/releases/3.13.2.tar.gz
tar -xf 3.13.2.tar.gz
cd Ipopt-releases-3.13.2
./configure --prefix=$HOME/usr # This should detect the HSL we installed above...
make install
```

Finally, install cyipopt:
```
pip3 install ipopt
```
